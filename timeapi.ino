...
// LED setup
#define NUM_LEDS 52  // Number of LEDs
...
WiFiUDP ntpUDP;
NTPClient timeClient(ntpUDP, "pool.ntp.org", 19800);  // Set IST timezone offset (5.5 hours = 19800 seconds)
...
void setup() {
    ...
    timeClient.begin();
    updateNTPTime();  // Ensure time is fetched before proceeding
    ...
    xTaskCreatePinnedToCore(ApiTask, "ApiTask", 16384, NULL, 2, &ApiTaskHandle, 1);  // Increased stack size to 16384
...
// Temperature Control Task (Sending random values for testing)
void TempControlTask(void *pvParameters) {
    double hysteresis = 0.1;  // Hysteresis buffer
    while (true) {
        // Generating random temperatures for testing
        double randomTemp = 36.0 + (rand() % 100) / 100.0;  // Random temperature between 36.0 and 37.99
...
// Helper function for handling HTTP requests with retry
void httpRequestWithRetry() {
    HTTPClient http;
    WiFiClient client;
    int maxRetries = 3;
    ...
    while (retryCount < maxRetries) {
        checkWiFiConnection();
        if (WiFi.status() == WL_CONNECTED) {
            updateNTPTime();  // Ensure NTP time is up to date
            ...
            if (averageTemp >= 0 && averageTemp <= 50) {  // Validate temperature range
                ...
                // Send random sensor temperatures for testing
                for (int i = 0; i < 4; i++) {
                    JsonObject tempValueObj = values.createNestedObject();
                    tempValueObj["key"] = String("Sensor_") + (i + 1);
                    tempValueObj["value"] = 36.0 + (rand() % 100) / 100.0;  // Random temperature between 36.0 and 37.99
                ...
                break;  // Exit loop if data is invalid
            }
        } else {
    ...
    if (retryCount == maxRetries) {
        Serial.println("Max retries reached. HTTP request failed.");
    }
}
...
// Helper function to update NTP time
void updateNTPTime() {
    timeClient.update();
    unsigned long epochTime = timeClient.getEpochTime();
    if (epochTime == 0) {
        Serial.println("Failed to get time from NTP server. Retrying...");
        timeClient.forceUpdate();  // Force an update if the initial one fails
    } else {
        Serial.println("NTP time updated successfully.");
    }
}

// Helper function to get formatted time in "YYYY-MM-DD HH:MM:SS" format
String getFormattedTime() {
    unsigned long epochTime = timeClient.getEpochTime();
...
// Helper function to set the LED color for all LEDs
void setLEDColor(uint8_t r, uint8_t g, uint8_t b) {
